...
namespace WabbitC_Tests
{
	...
	public class TokenizerTest
	{
		...
		[TestMethod()]
		public void TokenizeTest_VarStore()
		{
			List<Token> tokens = Tokenizer.Tokenize("int var = test;");

			Assert.AreEqual(TokenType.StringType, tokens[3].Type);
			Assert.AreEqual("test", tokens[3].Text);
		}

		[TestMethod()]
		public void TokenizeTest_SingleLineComment()
		{
			string commentString = "//test comment";
			List<Token> tokens = Tokenizer.Tokenize(commentString);

			Assert.AreEqual(TokenType.CommentType, tokens[0].Type);
			Assert.AreEqual(commentString, tokens[0].Text);
		}

		[TestMethod()]
		public void TokenizeTest_MultilineComment()
		{
			string commentString = "/*test comment*/";
			List<Token> tokens = Tokenizer.Tokenize(commentString);
		...
		[TestMethod()]
		public void TokenizeTest_MutilineComment_Multiline()
		{
			string commentString = "/*test comment\r\nmultiline comment*/";
			List<Token> tokens = Tokenizer.Tokenize(commentString);
		...
		[TestMethod()]
		public void TokenizeTest_MultilineComment_NoEnd()
		{
			string commentString = "/*test comment\r\nmultiline comment";
			List<Token> tokens = Tokenizer.Tokenize(commentString);
		...
		[TestMethod()]
		public void TokenizeTest_MultilineIf()
		{
			string ifString = "if (test && \r\n test2) { ; }";
			List<Token> tokens = Tokenizer.Tokenize(ifString);

			Assert.AreEqual(TokenType.ReservedKeyword, tokens[0].Type);
			Assert.AreEqual(TokenType.StatementEnd, tokens[7].Type);
		}
	}
}
