...
//			autoClearSelection = false;
		...
		{
		    return line < document.TotalNumberOfLines - 1 ? new TextLocation(0, line + 1) : new TextLocation(document.GetLineSegment(document.TotalNumberOfLines - 1).Length + 1, line);
		}
...
//				ISelection s = ((ISelection)queue.Remove());
                ...
                offset = s.Offset;//< s.EndOffset ? s.Offset : s.EndOffset;
				document.Remove(s.Offset, s.Length);
                if (textArea.Caret.Offset > s.Offset)
                    textArea.Caret.Column -= textArea.Caret.Offset - s.Offset;
