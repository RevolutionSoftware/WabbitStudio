...
namespace Revsoft.TextEditor.Document
{
	...
	public class DefaultFormattingStrategy : IFormattingStrategy
	{
		...
		/// as a string.
		/// </summary>
		public string GetIndentation(IDocument document, int lineNumber)
		{
		...
		/// </summary>
		protected virtual int AutoIndentLine(IDocument document, int lineNumber)
		{
			string indentation = lineNumber != 0 ? GetIndentation(document, lineNumber - 1) : "";
			if(indentation.Length > 0) {
				...
				SmartReplaceLine(document, oldLine, newLineText);
			}
		...
		/// Could be overwritten to define more complex indenting.
		/// </summary>
		protected virtual int SmartIndentLine(IDocument document, int line)
		{
			return AutoIndentLine(document, line); // smart = autoindent in normal texts
		}
		...
		/// </returns>
		public int IndentLine(IDocument document, int line)
		{
			document.UndoStack.StartUndoGroup();
			int result;
			switch (document.TextEditorProperties.IndentStyle) {
				case IndentStyle.None:
					result = 0;
					break;
				case IndentStyle.Auto:
					result = AutoIndentLine(document, line);
					break;
				case IndentStyle.Smart:
					result = SmartIndentLine(document, line);
					break;
				default:
					throw new NotSupportedException("Unsupported value for IndentStyle: " + document.TextEditorProperties.IndentStyle);
			}
			document.UndoStack.EndUndoGroup();
			return result;
		}
		...
		public virtual void IndentLines(TextArea textArea, int begin, int end)
		{
			...
			for (int i = begin; i <= end; ++i) {
				IndentLine(textArea.Document, i);
			}
