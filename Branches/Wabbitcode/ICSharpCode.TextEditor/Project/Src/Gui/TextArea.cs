...
//			SetStyle(ControlStyles.UserPaint, true);
		...
		{
			if (toolTip == null)// || toolTip.didIsDisposed)
                toolTip = new ToolTip();
			...
			if (text == null) {
				...
				toolTip.Show(text.Trim(), this, cp);
			}
			oldToolTip = text;
		}
...
//				clipRectangle.Intersect(updateMargin.DrawingPosition);
			...
			foreach (AbstractMargin margin in leftMargins) {
			    ...
			    if (!clipRectangle.IntersectsWith(marginRectangle)) continue;
		...
		internal bool IsReadOnly(int offset)
		{
			...
			return TextEditorProperties.SupportReadOnlySegments && Document.MarkerStrategy.GetMarkers(offset).Exists(m=>m.IsReadOnly);
		}
		...
		internal bool IsReadOnly(int offset, int length)
		{
			...
			return TextEditorProperties.SupportReadOnlySegments && Document.MarkerStrategy.GetMarkers(offset, length).Exists(m=>m.IsReadOnly);
		}
