...
namespace Revsoft.TextEditor
{
	...
	{
	    readonly StringFormat numberStringFormat = (StringFormat)StringFormat.GenericTypographic.Clone();
		
		public static Cursor RightLeftCursor;
		
		static GutterMargin()
		{
			Stream cursorStream = Assembly.GetCallingAssembly().GetManifestResourceStream("Revsoft.TextEditor.Resources.RightArrow.cur");
		...
		public override Size Size {
			get {
				return new Size(textArea.TextView.WideSpaceWidth * (1 + Math.Max(3, (int)Math.Log10(textArea.Document.TotalNumberOfLines) + 1)), -1);
			}
		}
