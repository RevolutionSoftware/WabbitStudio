...
namespace Revsoft.TextEditor
{
	...
	public class TextAreaControl : Panel
	{
        ...
        internal Button splitterButton = new Button();
		TextArea   textArea;
		...
		public TextAreaControl(TextEditorControl motherTextEditorControl)
		{
            ...
            splitterButton.FlatStyle = FlatStyle.Popup;
            splitterButton.FlatAppearance.BorderSize = 0;
            splitterButton.FlatAppearance.CheckedBackColor = SystemColors.Control;
            splitterButton.FlatAppearance.MouseDownBackColor = SystemColors.Control;
            splitterButton.FlatAppearance.MouseOverBackColor = SystemColors.Control;
            splitterButton.MouseDown += new MouseEventHandler(splitterButton_MouseDown);
            splitterButton.Anchor = AnchorStyles.Right | AnchorStyles.Top;
            Controls.Add(this.splitterButton);
        ...
        void splitterButton_MouseDown(object sender, MouseEventArgs e)
        {
            motherTextEditorControl.Split();
        }
		...
		public void ResizeTextArea()
		{
			...
			int h = 0;
			if (hRuler != null) {
				hRuler.Bounds = new Rectangle(0, 0, Width - SystemInformation.HorizontalScrollBarArrowWidth,
			...
			textArea.Bounds = new Rectangle(0, y, Width - SystemInformation.HorizontalScrollBarArrowWidth,
		...
		{
            splitterButton.Bounds = new Rectangle(textArea.Bounds.Right + 1, 1, SystemInformation.HorizontalScrollBarArrowWidth - 2, 6);
            if (motherTextEditorControl.TextEditorProperties.IsSplit)
                ...
                vScrollBar.Bounds = new Rectangle(textArea.Bounds.Right, 7, SystemInformation.HorizontalScrollBarArrowWidth, Height - SystemInformation.VerticalScrollBarArrowHeight);
			hScrollBar.Bounds = new Rectangle(0, textArea.Bounds.Bottom, Width - SystemInformation.HorizontalScrollBarArrowWidth,
				...
				if (line + scrollMarginHeight - 1 > curLineMax) {
					if (this.textArea.TextView.VisibleLineCount == 1) {
						...
						this.vScrollBar.Value = Math.Min(this.vScrollBar.Maximum,
                                                         (line - this.textArea.TextView.VisibleLineCount + scrollMarginHeight + (3 * this.textArea.TextView.VisibleLineCount) / 4) * textArea.TextView.FontHeight);
					}
					VScrollBarValueChanged(this, EventArgs.Empty);
				}
			}
