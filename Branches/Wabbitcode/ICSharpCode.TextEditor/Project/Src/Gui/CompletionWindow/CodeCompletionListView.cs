...
namespace Revsoft.TextEditor.Gui.CompletionWindow
{
	...
	{
	    readonly ICompletionData[] completionData;
		int               firstItem;
		int               selectedItem = -1;
		ImageList         imageList;
...
//			SetStyle(ControlStyles.DoubleBuffer, false);
		...
		{
			int oldFirstItem = FirstItem;
			int item = index - MaxVisibleItem / 2;
			if (item < 0)
				FirstItem = 0;
			else if (item >= completionData.Length - MaxVisibleItem)
			...
			for (int i = 0; i < completionData.Length; ++i) {
				...
				if (lowerText.StartsWith(startText)) {
					...
					if (lowerText == startText) {
						quality = itemText == originalStartText ? 3 : 2;
