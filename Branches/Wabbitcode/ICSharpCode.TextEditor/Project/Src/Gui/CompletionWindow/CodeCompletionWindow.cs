...
namespace Revsoft.TextEditor.Gui.CompletionWindow
{
	public class CodeCompletionWindow : AbstractCompletionWindow
	{
		...
		int startOffset;
		int endOffset;
		DeclarationViewWindow declarationViewWindow;
		...
		void SetDeclarationViewLocation()
		{
			...
			if (rightSpace * 2 > leftSpace) {
				...
				pos = Bounds.Left < declarationViewWindow.Width ? new Point(0, Bounds.Top) : new Point(Bounds.Left - declarationViewWindow.Width, Bounds.Top);
				if (declarationViewWindow.Location != pos) {
					declarationViewWindow.Location = pos;
				}
		...
		protected override sealed void SetLocation()
		{
			base.SetLocation();
			if (declarationViewWindow != null) {
				SetDeclarationViewLocation();
	    ...
	    readonly Util.MouseWheelHandler mouseWheelHandler = new Util.MouseWheelHandler();
